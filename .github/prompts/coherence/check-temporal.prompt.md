...
1.  **Reality Check (Read the Code)**
    - Scan `src/` to determine what *actually* exists and works.
...
3.  **Plan Audit (Update the Plan)**
    - **Mark Completed**: If a task in `docs/agent-context/plan.toml` is implemented, mark it as `status = "completed"`.
    - **Resurrect Pending**: If a task is marked `completed` but the code is missing or broken, revert it to `status = "proposed"` or `status = "active"`.
