...
/**
 * @summary List Project Domains by Apex Domain
 * @description List all project domains associated with an apex domain owned by the authenticated account.
 * @link /v1/domains/{domain}/project-domains
 */
export async function getDomainProjectDomains(
	{
		pathParams,
		queryParams,
		config,
	...
	>({
		method: "GET",
		url: `/v1/domains/${pathParams.domain}/project-domains`,
		queryParams,
		...requestConfig,
		headers: { ...requestConfig.headers },
	});
 ...
 * @description Sign a JWT with a KMS issuer's active signing key. Authenticate the request with a Vercel OIDC token in the `Authorization: Bearer` header; the issuer's policies decide which workloads are allowed to sign. `keyId`, `algorithm`, and `fingerprint` identify the signing key in the issuer's JWKS.
...
export const operationsByPath = {
	...
	"GET /v1/domains/{domain}/project-domains": getDomainProjectDomains,
