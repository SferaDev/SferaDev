...
import { Camera, Check, Settings, Trash2, UserPlus, X } from "lucide-react";
...
function TableNodeComponent({ data }: TableNodeProps) {
		...
		if (table.shape === "round") {
			...
			if (arrangement === "single-row") {
				...
				for (let i = 0; i < table.seats; i++) {
					const x =
						centerX -
						((table.seats - 1) * (seatSize + seatSpacing)) / 2 +
						i * (seatSize + seatSpacing);
				...
				for (let i = 0; i < topSeats && seatIndex < table.seats; i++) {
					const x =
						centerX -
						((topSeats - 1) * (seatSize + seatSpacing)) / 2 +
						i * (seatSize + seatSpacing);
				...
				for (let i = 0; i < bottomSeats && seatIndex < table.seats; i++) {
					const x =
						centerX -
						((bottomSeats - 1) * (seatSize + seatSpacing)) / 2 +
						i * (seatSize + seatSpacing);
					const y = centerY + tableHeight / 2 + seatSize / 2 + 16;
					const guest = guests.find((g) => g.seatIndex === seatIndex);
					seats.push({
				...
				for (let i = 0; i < rightSeats && seatIndex < table.seats; i++) {
					...
					const y =
						centerY -
						((rightSeats - 1) * (seatSize + seatSpacing)) / 2 +
						i * (seatSize + seatSpacing);
					const guest = guests.find((g) => g.seatIndex === seatIndex);
					seats.push({
						index: seatIndex,
				...
				for (let i = 0; i < leftSeats && seatIndex < table.seats; i++) {
					...
					const y =
						centerY +
						((leftSeats - 1) * (seatSize + seatSpacing)) / 2 -
						i * (seatSize + seatSpacing);
					const guest = guests.find((g) => g.seatIndex === seatIndex);
					seats.push({
						index: seatIndex,
	...
	return (
		...
		>
								...
								<Select
									...
									onValueChange={(v) =>
										onUpdateTable(table.id, {
											seatArrangement: v as "around" | "one-side" | "single-row",
										})
									}
								>
									<SelectTrigger>
			...
			{layout.isRound ? (
				<div
					className="absolute bg-linear-to-br from-primary/20 to-primary/10 border-[3px] border-primary/50 rounded-full flex items-center justify-center shadow-lg"
					style={{
						width: tableRadius * 2,
				...
				<div
					className="absolute bg-linear-to-br from-primary/20 to-primary/10 border-[3px] border-primary/50 rounded-2xl flex items-center justify-center shadow-lg"
					style={{
						width: tableSize?.width ?? 0,
						height: tableSize?.height ?? 0,
						left: center.x - (tableSize?.width ?? 0) / 2,
						top: center.y - (tableSize?.height ?? 0) / 2,
					}}
				>
					<div className="text-center px-3">
						{isEditing ? (
							...
							(() => {
								...
								return (
									<div
										...
										style={{
											opacity: draggingGuestId === guest.id ? 0.5 : 1,
										}}
									>
										<Avatar
										...
										>
											...
											<input
												...
												onChange={(e) => {
													...
													if (file) handlePhotoUpload(guest.id, file);
													e.target.value = "";
												}}
											/>
										</label>
